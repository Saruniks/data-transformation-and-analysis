...
async function main() {
    try {
        const tmpFiles = await downloadSpotifyDataset();

        const artists = await parseCSVFile(`${tmpFiles.name}/artists.csv`);
        logger.trace('Example artists row:', artists[0]);

        const tracks = await parseCSVFile(`${tmpFiles.name}/tracks.csv`);
